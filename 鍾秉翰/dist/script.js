...
function renderMain(){
  ...
  groupData.forEach((group,index)=>{
    // ★ 準備判斷整個群組的項目/參數內容
    let currentContent = group.items.map(item => [
      item.name || "",
      item.type || "數值",
      item.unit || "",
      item.max ?? "",
      ...
      item.min ?? "",
      item.manual ? "✔" : ""
    ].join("|")).join("||"); // 用 || 分隔每列，整個群組用一個字串表示

    // ★ 判斷是否跟其他群組相同
    let isDuplicateGroup = groupData.some((g,i) => i !== index && 
      g.items.map(item => [
        item.name || "",
        item.type || "數值",
        item.unit || "",
        ...
        item.min ?? "",
        item.manual ? "✔" : ""
      ].join("|")).join("||") === currentContent
    );

    // ★ 項目參數 HTML
    let itemHTML=`
    <table class="sub-table" style="display:none; color:${isDuplicateGroup ? "red" : "white"}">
      <thead>
        <tr>
...
function addProcess() {
    ...
    let name = ""
    list.push({ id: nextId, name: name,createdAt: new Date() });
    renderProcess();
    renderMain();
}
...
function renderItem() {
  ...
  list.forEach((item, i) => {
    ...
    tr.innerHTML = `
      ...
      <td>
        <select class="unit-select">
          ...
          <option value="gf" ${item.unit === "gf" ? "selected" : ""}>gf</option>
          <option value="mm" ${item.unit === "mm" ? "selected" : ""}>mm</option>
        </select>
        <input type="text" class="unit-input" value="${item.unit}" style="display:${["","manual"].includes(item.unit) ? "inline-block" : "none"}; width:60px;">
      </td>
    ...
    // ====== 編輯欄位更新時間 ======
    let editableTDs = tr.querySelectorAll("td[contenteditable='true']");
    editableTDs.forEach((td, idx) => {
      td.addEventListener("input", () => {
        let keys = ["name", "unit", "max", "mid", "min"];
        item[keys[idx]] = td.innerText.trim();
        item.createdAt = new Date();
        tr.children[8].innerText = formatDate(item.createdAt);
        renderMain();
      });
    });

    // ====== type 下拉選單 ======
    let selectType = tr.querySelector(".format-select");
    ...
    let unitInput = tr.querySelector(".unit-input");
...
function renderMaterial() {
  ...
  list.forEach((m, i) => {
    ...
    tr.children[1].addEventListener("input", (e) => {
      ...
      m.createdAt = new Date();     // 更新時間
      renderMain();                 // 即時同步主畫面
      tr.children[4].innerText = formatDate(m.createdAt); // 更新畫面上的時間
    });
