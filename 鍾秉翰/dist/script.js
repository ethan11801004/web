...
function importExcel(event){
  ...
  reader.onload=function(e){
    ...
    wb.SheetNames.forEach(sheet=>{
      ...
      if(sheet.includes("_Process")){
        let code=sheet.replace("_Process","");
        let g=groupData.find(g=>g.groupCode===code);
        if(!g){ g={groupCode:code,process:[],items:[],materials:[]}; groupData.push(g);}
        g.process=arr.slice(1).map(r=>r[0]||"");
      } else if(sheet.includes("_Item")){
        let code=sheet.replace("_Item","");
        let g=groupData.find(g=>g.groupCode===code);
        if(!g){ g={groupCode:code,process:[],items:[],materials:[]}; groupData.push(g);}
        g.items=arr.slice(1).map(r=>({id:r[0],name:r[1],type:r[2],unit:r[3],max:r[4],mid:r[5],min:r[6],manual:r[7]==="TRUE"}));
      } else if(sheet.includes("_Material")){
        let code=sheet.replace("_Material","");
        let g=groupData.find(g=>g.groupCode===code);
        if(!g){ g={groupCode:code,process:[],items:[],materials:[]}; groupData.push(g);}
        g.materials=arr.slice(1).map(r=>r[0]);
      }
    });
    renderMain();
  }
